\input{util/setup.tex}

%% This is the space for custom commands for your Formelsammlung
\newcommand{\FormelsammlungTitel}{High-frequency, Components, Amplifiers and Oscillators}
\newcommand{\FormelsammlungAutor}{Bogdan Stamenic}
\setcounter{tocdepth}{2} % Show only sections and subsection in table of contents

\begin{document}
	\title{\FormelsammlungTitel}
	\author{\FormelsammlungAutor}
	\date{\today}
	\begin{multicols}{3}
        {\small%
%		\begin{minipage}{.4\paperheight}
			\maketitle
			\tableofcontents
%		\end{minipage}
        \input{content/hfcomp/wave_parameters_and_passive_high-frequency_components.tex}
        %\input{content/hfcomp/active_circuit_elements.tex}
        %\input{content/hfcomp/hf_amplification.tex}
        %\input{content/hfcomp/electronic_noise.tex}
        %\input{content/hfcomp/rf_two-port_amplifiers.tex}
        %\input{content/hfcomp/oscillators.tex}
        %\input{content/hfcomp/transceiver_concepts_and_system_considerations.tex}
        }
	\end{multicols}
\end{document}
