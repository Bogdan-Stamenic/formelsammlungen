...
\begin{itemize}
    \item Temporal behaviour is fundamentally \textit{unpredictable} $\implies$ integral values are predictable:
        \begin{align}
            ...
            &\text{Root Mean Square (RMS):} &\tilde{u}=\sqrt{\overline{u^2}}
        \end{align}
    ...
    \item \textbf{(Normalized) Error Function:}
        \begin{equation*}
            p(0<u<u_1) = \int\limits_0^{u_1} p^*(u)\mathrm{d}u = \Phi^*\left(\dfrac{u_1}{\tilde{u}}\right) \approx \dfrac{1}{2} - \dfrac{\exp(-\frac{1}{2}\frac{u_1^2}{\tilde{u}})}{\sqrt{2\pi}(u_1/\tilde{u})}
        \end{equation*}
